...
float EPSILON = 0.0001;// * (1.+sin(iTime));
...
float cubeSDF(vec3 p) {
    ...
    float insideDistance = min(max(d.x, max(d.y, d.z)), 0.0); //when the max is less than 0
...
vec3 phongContribForLight(vec3 k_d, vec3 k_s, float alpha, vec3 p, vec3 eye,
    ...
    float dotLN = dot(L, N); // light dot normal
    float dotRV = dot(R, V); // reflection dot view

    if (dotLN < 0.0) {
        // Light not visible from this point on the surface
        // AKA shadow area
        return vec3(0);
        // game over for the pixel
    }

    if (dotRV < 0.0) {
        // Light reflection in opposite direction as viewer, apply only diffuse
        // component
        // AKA no reflected lights returned
        return lightIntensity * (k_d * dotLN);
        // game over for the pixel
    }
    return lightIntensity * (k_d * dotLN + k_s * pow(dotRV, alpha));
    //return lightIntensity * (k_d + k_s * pow(dotRV, alpha));
    //return vec3(1) * (dotLN+pow(dotRV, alpha));
}
 ...
 * See https://en.wikipedia.org/wiki/Phong_reflection_model#Description
 */
 // use phong model to do the real lighting setup
 ...
 */
mat4 viewMatrixNew(vec3 eye, vec3 center, vec3 up) {
...
//    vec3 eye = vec3(0.0, 0.0, 10.); // 5 on z axis
//    float dist = shortestDistanceToSurface(eye, dir, MIN_DIST, MAX_DIST);

    vec3 viewDir = rayDirection(45.0, iResolution.xy, gl_FragCoord.xy);
    vec3 eye = vec3(4., 4., 10.);
    mat4 viewToWorld = viewMatrixNew(eye, vec3(0.0, 0.0, 0.0), vec3(0.0, 1.0, 0.0));
